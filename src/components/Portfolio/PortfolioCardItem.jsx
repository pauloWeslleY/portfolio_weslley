...
export function PortfolioCardItem(props) {
	...
	return (
		...
		>
			...
			<Stack pt={10} align={'center'}>
				...
				<chakra.p mt={1} fontSize={'1.5rem'} color={THEME.PORTFOLIO_PARAGRAPH_COLOR}>
					{tech}
				</chakra.p>
				<Box>{techlist}</Box>
				<Flex alignItems={'center'} justifyContent={'center'} gap={2} px={4} py={2}>
					<Btn name={livePreview} link={link} icon={<BiLinkExternal />} />
					<Btn name={git} link={github} icon={<BsGithub />} />
				</Flex>
				<PortfolioModal
					title={'Pré visualização do Site'}
					image={<Image rounded={'lg'} fit={'cover'} my={2} src={image} alt={name} />}
