...
const CardContact = ({ icon, name, label }) => (
	<Flex
		direction={'column'}
		align={'center'}
		basis={'15rem'}
	...
	>
		...
		<Heading fontSize={{ base: 'smaller', sm: '2rem' }} fontFamily={'body'} mb={'0.9rem'}>
			{name}
		</Heading>
		<Text
			as={'span'}
	...
	return (
		<section>
			<Flex flexWrap={'wrap'} w={{ base: '100%', lg: '90rem' }}>
				{CARDS.map(item => (
					<CardContact key={item.id} name={item.name} label={item.label} icon={item.icon} />
				))}
			</Flex>
		</section>
	);
